...
/**
 * Class BLUISHOST_Font_Container
 *
 * @since         4.3
 *                vc_map examples:
 *    array(
 ...
 *                'color',
 *
 *                'tag_description' => __('Select element tag.','bluishost'),
 *                'text_align_description' => __('Select text alignment.','bluishost'),
 *                'font_size_description' => __('Enter font size.','bluishost'),
 *                'line_height_description' => __('Enter line height.','bluishost'),
 *                'color_description' => __('Select color for your element.','bluishost'),
 *            ),
 *        ),
 *    ),
...
class BLUISHOST_Font_Container {
	...
	function __construct() {
		if ( function_exists( 'vc_add_shortcode_param' ) ) {
			vc_add_shortcode_param( 'bluishost_font_container', array( $this, 'vc_font_container_form_field' ),  plugins_url( '/js/bluishost_additional_param.js', __FILE__ ) );
		}
	...
	function vc_font_container_form_field( $settings, $value ) {
		$font_container = new BLUISHOST_Font_Container();
	...
	public function render( $settings, $value ) {
		$fields = array();
		$values = array();
		extract( $this->_bluishost_font_container_parse_attributes( $settings['settings']['fields'], $value ) );

		$data   = array();
		$output = '<div class="vc_row bluishost_vc">';
		if ( ! empty( $fields ) ) {
			...
			if ( isset( $fields['tag'] ) ) {
                ...
                <div class="vc_col-' . $columns . ' ">
                    <div class="wpb_element_label">' . __( 'Element tag', 'bluishost' ) . '</div>
				...
				$tags        = $this->_bluishost_font_container_get_allowed_tags();
				foreach ( $tags as $tag ) {
			...
			if ( isset( $fields['font_size'] ) ) {
				$data['font_size'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Font size', 'bluishost' ) . '</div>
			...
			if ( isset( $fields['line_height'] ) ) {
				$data['line_height'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Line height', 'bluishost' ) . '</div>
			...
			if ( isset( $fields['color'] ) ) {
				$data['color'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Text color', 'bluishost' ) . '</div>
			...
			if ( isset( $fields['letter_spacing'] ) ) {
				$data['letter_spacing'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Letter spacing', 'bluishost' ) . '</div>
			...
			if ( isset( $fields['font_style'] ) ) {
				...
				$data['font_style'] .= '<div class="wpb_element_label">' . __( 'Font style', 'bluishost' ) . '</div>';
				...
				if(isset($values['font_style_italic'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label italic"> ' . esc_html__( 'Italic', 'bluishost' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
				...
				if(isset($values['font_style_underline'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label underline"> ' . esc_html__( 'Underline', 'bluishost' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
				...
				if(isset($values['font_style_bold'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label bold"> ' . esc_html__( 'Bold', 'bluishost' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
	 ...
	 */
	public function _bluishost_font_container_get_allowed_tags() {
		$allowed_tags = array(
			'h1',
			'h2',
			'h3',
		...
		return apply_filters( 'bluishost_font_container_get_allowed_tags', $allowed_tags );
	 ...
	 * @return array
	 */
	public function _bluishost_font_container_parse_attributes( $attr, $value ) {
		$fields = array();
		if ( isset( $attr ) ) {
...
if ( class_exists( 'BLUISHOST_Font_Container' ) ) {
	$bluishost_font_container = new BLUISHOST_Font_Container();
}
