<?php
if(!class_exists("BluishostBase")) {
	class BluishostBase {
    	public $key = "A056D7F01C67CC27";
    	private $product_id = "24";
    	private $product_base = "bluishost";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_bluishost_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=bluishost_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
